                    ...
                    if (!userExistsInRoom(roomObj, socket.id)) {
                        ...
                        if (roomObj.users.length == 2) {
                            ...
                            for(let i = 0; i < pokerUsers.length; i++)
                                if(pokerUsers[i].index == roomIndex)
                                    pokerUsers[i].full = 1; 
                            fs.readFile(jsonFilePath2, 'utf8', (err, fileData) => {
                                if (err) {
                                    console.error('Error reading the JSON file:', err);
        ...
        fs.readFile(jsonFilePath2, 'utf8', (err, fileData) => {
            ...
            try {
                ...
                if (jsonData[socket.serverIndex]) {
                    ...
                    if (jsonData[userToRemove.index].user1 === userToRemove.user) {
                        console.log(`Clearing user1 slot: ${jsonData[userToRemove.index].user1}`);
                        jsonData[userToRemove.index].user1 = '';
                    } else if (jsonData[userToRemove.index].user2 === userToRemove.user) {
                        console.log(`Clearing user2 slot: ${jsonData[userToRemove.index].user2}`);
                        jsonData[userToRemove.index].user2 = '';
                    } else if (jsonData[userToRemove.index].user3 === userToRemove.user) {
                        console.log(`Clearing user3 slot: ${jsonData[userToRemove.index].user3}`);
                        jsonData[userToRemove.index].user3 = '';
                    } else if (jsonData[userToRemove.index].user4 === userToRemove.user) {
                        console.log(`Clearing user4 slot: ${jsonData[userToRemove.index].user4}`);
                        jsonData[userToRemove.index].user4 = '';
                    } else {
                        console.log(`No matching user slot found for ${userToRemove.user} in index ${userToRemove.index}`);
                    }
